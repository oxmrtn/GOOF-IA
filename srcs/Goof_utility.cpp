...
int Goof::utility_easy(Game state) // Utility func for easy mode
{
    ...
    score -= (mult * state.aligned_piece(  1 - this->player )) * 2;
...
int Goof::utility_medium(Game state) // Utility func for medium mode
{
    ...
    score -= (state.piece_value( 1 - this->player) * mult) * 2; 
    score += (state.count_threat( 1 - this->player ) * mult) * 2; // Count the threat 
    return (score);
}
...
int Goof::utility_hard(Game state) // Utility func for hard mode
{
    ...
    score -= (state.remaining_piece( 1 - this->player) * mult); 
    score += (state.detect_fork((this->player)) * mult); // Detect potential fork on the board
    score -= (state.detect_fork( (1 - this->player )) * mult); // Detect potential fork for the opponent
    return (score);
}